...
LRESULT CView::OnInitDialog(HWND hwndFocus, LPARAM lParam) {
  ...
  for (; i < format_.column_name.size() - 1; i++) {
    list_.InsertColumn(i + 1, format_.column_name[i].c_str(), LVCFMT_LEFT,
                       width[i], i + 1);
  }
  list_.InsertColumn(i + 1, format_.column_name[i].c_str(), LVCFMT_LEFT, 700,
                     i + 1);
...
COLORREF level_colour[] = {RGB(0xBB, 0xBB, 0xBB), RGB(0x00, 0x70, 0xBB),
                           RGB(0x48, 0xBB, 0x31), RGB(0xBB, 0xBB, 0x23),
                           RGB(0xFF, 0x00, 0x06), RGB(0x8F, 0x00, 0x05)};
LRESULT CView::OnNMCustomdrawList(int, LPNMHDR pNMHDR, BOOL &) {
  ...
  switch (lplvcd->nmcd.dwDrawStage) {
    ...
    case CDDS_ITEMPREPAINT:  // Before an item is drawn
    {
      ...
      ReleaseMutex(mutex_);
      lplvcd->clrText = level_colour[tool::Levelstoi(
          format_.level_text, log.GetMember(format_.index_column_level))];
      if (log.mark) {
...
static int focused;
LRESULT CView::OnNMRclickViewList(int idCtrl, LPNMHDR pNMHDR,
                                  BOOL & /*bHandled*/) {
  ...
  if (format_.index_column_interval >= 0) {
    rclick_menu.AppendMenuA(
        MF_STRING, ID_RCLICK_INTERVAL_FROM,
        (format_.column_name[format_.index_column_interval] + " from this")
            .c_str());
    rclick_menu.AppendMenuA(
        MF_STRING, ID_RCLICK_INTERVAL_TO,
        (format_.column_name[format_.index_column_interval] + " to this")
            .c_str());
    rclick_menu.AppendMenuA(MF_SEPARATOR);
  }
  rclick_menu.AppendMenuA(MF_STRING, ID_RCLICK_ADDTO_EDIT, "add to find");
  rclick_menu.AppendMenuA(MF_STRING, ID_RCLICK_SAVEDATA_FILE, "save to file");
...
LRESULT CView::OnRclickMenu(WORD, WORD id, HWND, BOOL &) {
  ...
  switch (id) {
    ...
    case ID_RCLICK_INTERVAL_FROM: {
      WaitForSingleObject(mutex_, INFINITE);
      control_.AddInterval(
          focused_log.GetMember(format_.index_column_interval));
      control_.RefreshFilter();
      ReleaseMutex(mutex_);
...
LRESULT CView::OnEnKillfocusEdit(WORD wNotifyCode, WORD wID, HWND hWndCtl,
                                 BOOL &bHandled) {
...
LRESULT CView::OnNMClickViewList(int /*idCtrl*/, LPNMHDR pNMHDR,
                                 BOOL & /*bHandled*/) {
  LPNMITEMACTIVATE lpnmitem = (LPNMITEMACTIVATE)pNMHDR;
...
std::string CView::GetSaveFile() {
  ...
  ofn.lpstrDefExt = TEXT("log");  // extend file name
...
void CView::AllSaveFile(std::string name) {
  std::ofstream ofile;
  ofile.open(name);
  if (!ofile) {
    ofile.clear();
  ...
  for (int i = 0; i < control_.Size(); i++) {
    ofile << control_.GetLog(i).get_log()<<std::endl;
  }
  ofile.clear();
  ofile.close();
}
